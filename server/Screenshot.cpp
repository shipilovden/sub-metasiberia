...
void writeScreenshotToStream(const Screenshot& shot, OutStream& stream)
{
	...
	Screenshot::ScreenshotType effective_type = shot.screenshot_type;
	if(shot.is_map_tile && effective_type == Screenshot::ScreenshotType_Default)
		effective_type = Screenshot::ScreenshotType_MapTile;

	stream.writeUInt32((uint32)effective_type);
...
void readScreenshotFromStream(InStream& stream, Screenshot& shot)
{
	...
	if(v >= 4)
	{
		...
		shot.is_map_tile = (shot.screenshot_type == Screenshot::ScreenshotType_MapTile);
